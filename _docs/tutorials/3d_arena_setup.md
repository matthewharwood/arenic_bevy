# 3D Arena Setup Tutorial

This tutorial demonstrates how to set up a 3D arena grid system in Bevy for a game called "Arenic". The code creates a 3x3 grid of arenas with colored tiles and camera positioning.
...
fn setup_arena_grid(
    ...
    for arena_index in 0..TOTAL_ARENAS {
        ...
        // For arena 0, we want it exactly at the top-left corner of the window
        let arena_x = -HALF_WINDOW_WIDTH + x_offset + HALF_TILE;
        let arena_y = HALF_WINDOW_HEIGHT - y_offset - HALF_TILE;
...
/// Calculate camera position to center on a specific arena (updated for 3D consistency)
pub fn calculate_camera_position(arena_index: u8) -> (f32, f32) {
    let arena_col = arena_index % ARENAS_PER_ROW as u8;
    let arena_row = arena_index / ARENAS_PER_ROW as u8;

    // Calculate arena position (matching setup_arena_grid positioning)
    let x_offset = arena_col as f32 * ARENA_WIDTH;
    let y_offset = arena_row as f32 * ARENA_HEIGHT;

    let arena_x = -HALF_WINDOW_WIDTH + x_offset;
    let arena_y = HALF_WINDOW_HEIGHT - y_offset;

    // Calculate arena center by adding half arena dimensions
    let center_x = arena_x + ARENA_WIDTH / 2.0;
    let center_y = arena_y - ARENA_HEIGHT / 2.0;
...
fn setup_camera(commands: &mut Commands, arena_index: u8) {
    let (center_x, center_y) = calculate_camera_position(arena_index);
    // For arena 0: center should be at (-640 + 627, 360 - 294.5) = (-13, 65.5)
    let x = 0.0;
    let y = 0.0;
    commands.spawn((
        Camera3d::default(),
        Transform::from_xyz(x, y, 1000.0).looking_at(Vec3::new(x, y, 0.0), Vec3::Y),
...
1. **3x3 Arena Grid**: Creates 9 arenas arranged in a 3x3 grid
2. **Debug Visualization**: Each arena has a unique color from the `DEBUG_COLORS` array
3. **3D Orthographic Camera**: Uses an orthographic projection for a top-down view
4. **Lighting System**: Includes both directional and ambient lighting
5. **Component System**: Uses ECS components like `GridPosition`, `ArenaTile`, and `CurrentArena`

## Usage

This code sets up a basic 3D arena system suitable for strategy games, board games, or any game requiring a grid-based battlefield layout. The camera is positioned to view arena 1 (index 1) by default, but can be changed by modifying the `setup_camera` call.