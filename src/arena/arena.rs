use crate::arena::{LastActiveHero, TILE_SIZE};
...
pub fn move_active_character(
    ...
    use crate::arena::{ARENA_HEIGHT, ARENA_WIDTH, GRID_HEIGHT, GRID_WIDTH};
    ...
    if new_position.x < min_x {
        ...
        if col > 0 {
            ...
            if let Some((new_arena_entity, _)) = arena_q.iter().find(|(_, arena)| arena.0 == new_arena_index) {
                commands.entity(character_entity).insert(ChildOf(new_arena_entity));
            }
    ...
    println!("Character at: {:?} in arena {}", character_transform.translation, current_arena.0);
}
