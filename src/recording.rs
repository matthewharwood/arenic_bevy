...
use crate::components::{
    ActionEvent, ArenaName, ArenaStatus, ArenaTimer, Character, CharacterSelected, CharacterTimer,
    CurrentArena, PlaybackState, RecordedActions,
};
...
/// Helper function to set arena status to Recording and reset timer
fn setup_arena_for_recording(
        ...
        .find(|at| at.arena == *arena_name)
    {
        ...
        println!(
            "Arena {} status changed to Recording, timer reset",
            arena_name.name()
        );
    }
}
    ...
    fn build(&self, app: &mut App) {
        app.add_systems(
            ...
            (
                ...
                .chain()
                .before(crate::movement::handle_movement_input),
        )
        .add_systems(
            Update,
            (
            ...
            if character_timer.is_recording {
                ...
                if selected_query.get(entity).is_err() {
                    ...
                    if let Ok(mut recorded_actions) = recorded_actions_query.get_mut(entity) {
                        ...
                        // Show the appropriate message based on whether there's a saved session
                        if recorded_actions.has_saved_session(arena_index) {
                            println!(
                                "Tab pressed: Reverted to saved session for {} in arena {}",
                                character.name,
                                arena_name.name()
                            );
                        } else {
                            println!(
                                "Tab pressed: Reverted to empty state (no saved session) for {} in arena {}",
                                character.name,
                                arena_name.name()
                            );
                        }
        ...
        if let Ok(character_timer) = character_timer_query.get(entity) {
            if character_timer.is_recording {
                if let Ok(mut recorded_actions) = recorded_actions_query.get_mut(entity) {
                    ...
                    recorded_actions.clear_draft();
                    println!(
                        "Arena transition: Cancelled active recording for character {} entering {}",
                        character.name,
                        new_arena.name()
                    );
                }
...
fn handle_recording_toggle(
    ...
    if input.just_pressed(KeyCode::KeyR) {
        if let Ok((selected_entity, character, transform, arena_name)) = selected_query.single() {
            ...
            match (
                character_timer_query.get_mut(selected_entity),
                recorded_actions_query.get_mut(selected_entity),
            ) {
        ...
        if let Ok((character_timer, arena_name)) = character_query.get(event.entity) {
            if character_timer.is_recording {
                if let Ok(mut recorded_actions) = recorded_actions_query.get_mut(event.entity) {
                    ...
                    if added {
                        println!(
                            "Recorded movement: {:?} at t={:.2}s in arena {} (absolute timestamp)",
                            event.direction,
                            timestamp,
                            arena_name.name()
                        );
...
fn update_character_timers(mut character_timer_query: Query<&mut CharacterTimer>, time: Res<Time>) {
                ...
                .find(|at| at.arena == target_arena)
            {
                ...
                match arena_timer.get_status() {
                    ArenaStatus::Playback => {
                        ...
                        arena_timer.set_status(ArenaStatus::Paused);
                        println!(
                            "Arena {} status changed from Playback to Paused",
                            target_arena.name()
                        );
                    }
                    _ => {
                        ...
                        arena_timer.timer.reset(); // Reset timer to start from the beginning
...
fn start_playback_for_arena(
    ...
    for arena_timer in arena_timer_query.iter() {
        if arena_timer.is_playback() {
            ...
            for (entity, character, arena_name, recorded_actions, mut transform) in
                character_query.iter_mut()
            {
                if *arena_name == arena_timer.arena {
                    println!(
                        "Checking {} in arena {} for saved recordings",
                        character.name,
                        arena_name.name()
                    );
                    if let Some(recorded_actions) = recorded_actions {
                        if let Some(saved_recording) =
                            recorded_actions.get_saved_recording(arena_timer.arena.to_index())
                        {
                            println!(
                                "Found saved recording for {} with {} actions",
                                character.name,
                                saved_recording.actions.len()
                            );
                            if let Some(playback_state) =
                                PlaybackState::new(saved_recording.clone())
                            {
                                ...
                                println!(
                                    "Started playback for {} in arena {} with {} actions",
                                    character.name,
                                    arena_name.name(),
                                    saved_recording.actions.len()
                                );
                                println!(
                                    "  Recording start time: {:.2}s, end time: {:.2}s",
                            ...
                            println!(
                                "No saved recording found for {} in arena {}",
                                character.name,
                                arena_name.name()
                            );
                        }
                        ...
                        println!(
                            "No RecordedActions component for {} in arena {}",
                            character.name,
                            arena_name.name()
                        );
                    }
            ...
            ArenaStatus::Paused | ArenaStatus::Recording
        ) {
            // Remove playback states when an arena is no longer in playback mode
...
fn process_playback_actions(
    ...
    for (entity, mut playback_state, mut transform, character) in playback_query.iter_mut() {
        ...
        if let Some(arena_timer) = arena_timer_query
            .iter()
            ...
            while let Some(action) = playback_state.get_current_action(elapsed) {
                match action {
                    ActionEvent::Position { x, y, timestamp } => {
                        ...
                        transform.translation.y = *y;
                        println!(
                            "Playback: {} position set to ({}, {}) at relative time {:.2}s",
                            character.name,
                            x,
                            y,
                            timestamp - playback_state.recording.session_start_time
                        );
                    }
                    ActionEvent::Move {
                        direction,
                        ...
                        println!(
                            "Playback: {} move {:?} at relative time {:.2}s (elapsed: {:.2}s)",
                            character.name,
                            direction,
                            timestamp - playback_state.recording.session_start_time,
                            elapsed
...
fn stop_playback_when_complete(
    ...
    for (entity, playback_state) in playback_query.iter() {
        ...
        let still_in_playback = arena_timer_query
            .iter()
            .any(|at| at.arena == arena_name && at.is_playback());

        // Remove the playback state if complete or arena is no longer in playback
        if playback_state.is_complete() || !still_in_playback {
            commands.entity(entity).remove::<PlaybackState>();
            if playback_state.is_complete() {
                println!(
                    "Playback complete for entity in arena {}",
                    arena_name.name()
                );
            }
        }
